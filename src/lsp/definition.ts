...
import { DefinitionParams, LocationLink, Position, Range } from "vscode-languageserver";
...
export class DefinitionManager {
  ...
  async onDefinition(params: DefinitionParams, textDocument: TextDocument): Promise<LocationLink[] | null> {
  ...
  async getMarkdownLinkDefinition(
    ...
    const targetHeading = markdown.findElement((e) => e.type === "heading" && e.level === 1, target.elements);

    const targetRange =
      targetHeading == null ? Range.create(Position.create(0, 0), Position.create(0, 0)) : targetHeading;

    return [LocationLink.create(url.pathToFileURL(target.filePath).toString(), targetRange, targetRange, element.path)];
  }
  ...
  async getAssetDefinition(
    ...
    const targetPath = path.join(path.dirname(url.fileURLToPath(params.textDocument.uri)), element.path.content);

    if (!(await utils.isFileReadable(targetPath))) return null;

    const dummyRange = Range.create(Position.create(0, 0), Position.create(1, 1));

    return [LocationLink.create(url.pathToFileURL(targetPath).toString(), dummyRange, dummyRange, element.path)];
  }

  async getCitationDefinition(params: DefinitionParams, element: MarkdownCitation): Promise<LocationLink[] | null> {
    if (element.key == null || !markdown.isWithinRange(params.position, element.key)) return null;

    const target = await this.documentProvider.getDocumentByCitationKey(element.key.content);
    if (target == null) return null;
    ...
    return [LocationLink.create(url.pathToFileURL(target.filePath).toString(), targetRange, targetRange, element.key)];
  }
}
